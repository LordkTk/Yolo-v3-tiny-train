...
@author: cfd_Liu
"""
'This data_augmentation file is from wizyoungs repo and is slightly modified for my own data format'
import cv2
import numpy as np
import random
import os
...
def data_augmentation(imgSrc, bboxSrc, w, h, imgSize):
    ...
    boxes = bboxSrc.copy()
    img = random_color_distort(imgSrc)
    ...
    img, boxes = resize_with_bbox(img, boxes, imgSize, imgSize, interp=interp, letterbox=False)

    # random horizontal flip
    h, w, _ = img.shape
    img, boxes = random_flip(img, boxes, px=0.5)
    
    if not val_bbox(boxes):
        img = imgSrc
        boxes = bboxSrc
...
if __name__ == '__main__':
    path_train = np.load('./data/path_test.npy').item()[1]
    imgSize_train = 416
    imgInfo_train = np.load('./data/dataset_test.npy').item()
    trainList = np.load('./data/testList.npy').item()[1]
