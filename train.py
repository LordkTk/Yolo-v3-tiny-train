...
from prediction import post_process
...
#    trainList = testList + trainList  #use all imgs for training1
    ...
    no_obj_scale = tf.placeholder(tf.float32)
    coor_scale = tf.placeholder(tf.float32)
    obj_scale = tf.placeholder(tf.float32)
    class_scale = tf.placeholder(tf.float32)
    ...
    if warm_up:
        ...
        loss = cal_loss(out, [y1, y2], anchorCoor, bbox, no_obj_scale, coor_scale, obj_scale, class_scale, focal_loss=True)
        ...
        for epoch in range(epochMax):
            ...
            while sampleList != []:
                ...
                feed_dict = {x: imgbatch, y1: y[0], y2: y[1], bbox: bbox_gr,
                             is_training: True, coor_scale: scale[0], no_obj_scale: scale[1], 
                             obj_scale: scale[2], class_scale: scale[3]}
...
#    AP, mAP = train_yolo(scd_stage=True)
